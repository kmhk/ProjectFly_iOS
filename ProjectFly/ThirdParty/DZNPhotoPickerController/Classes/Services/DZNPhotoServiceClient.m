...
- (NSArray *)parseObjects:(Class)class withJSON:(NSDictionary *)json
{
    ...
    if (json[@"pagination"]) {
        if (json[@"pagination"][@"next_url"]) {
            self.nextPageUrl = json[@"pagination"][@"next_url"];
            _hasMore = YES;
        }
    }
...
{
    NSString *path;
    if (page > 1 && self.hasMore && self.nextPageUrl) {
        path = self.nextPageUrl;
    } else {
        path = photoSearchUrlPathForService(self.service);
    }

    NSDictionary *params = [self photosParamsWithKeyword:keyword page:page resultPerPage:resultPerPage];
    [params setValue:[NSNumber numberWithInt:24] forKey:@"count"];
