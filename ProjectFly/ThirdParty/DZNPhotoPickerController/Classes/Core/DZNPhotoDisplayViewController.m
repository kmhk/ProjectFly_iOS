...
- (instancetype)initWithCollectionViewLayout:(UICollectionViewLayout *)layout
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"Instagram Photos", nil);
...
- (UISearchBar *)searchBar
{
    if (!_searchBar)
    {
        ...
        _searchBar.text = [defaults objectForKey:@"instagram_userid"];
        //_searchBar.text = [NSString stringWithFormat:@"%@?count=28", [defaults objectForKey:@"instagram_userid"]];
...
- (void)setActivityIndicatorsVisible:(BOOL)visible
{
    ...
    if (visible) {
        ...
        if (!self.selectedServiceClient.hasMore) {
            self.loadButton.hidden = YES;
        }
    }
